...
modules.detection = function(smiles,
  ...
  clu_list=colMeans(clu_list)
  
  
  # Pick peaking
  loc.max <- 8 - apply(embed(clu_list, 7), 1,  which.max)
  loc.max[loc.max == 1 | loc.max == 7] <- NA
  pks <- loc.max + 0:(length(loc.max) - 1)
  clusters=unique(pks[!is.na(pks)])+1
  main_cluster=which.max(clu_list)+1
  
  print(paste("There are",main_cluster,"clusters"))
  if(length(clusters)>1){
    print(paste("There are other possible subclustering:",paste(clusters,collapse = ", ")))
  }
  ...
  return(list(visualization=res.kodama.visualization,clusters=clusters,
              silhouette=clu_list,max_nc=max_nc,min_nc=min_nc,
              hclust=res.hclust))
